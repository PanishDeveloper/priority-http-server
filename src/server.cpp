...
HttpServer::HttpServer(asio::io_context& ioc, unsigned short port, size_t numThreads, std::unique_ptr<LogSink> sink)
                       : m_ioc(ioc), m_port(port), m_pool(numThreads),
                         m_logger(std::move(sink)), m_signals(m_ioc, SIGINT, SIGTERM){}
...
void HttpServer::run()
{
    ...
    m_signals.async_wait([this](const boost::system::error_code& ec, int signal)
    {
        if (!ec)
        {
            ...
            m_logger.log("Shutdown signal received (SIG " + std::to_string(signal) + ")", LogLevel::INFO);
            m_ioc.stop();
        }
    });
    ...
    while (!m_shutdownRequested)
    {
        ...
        {
            if (e.code() == asio::error::operation_aborted || e.code() == boost::system::errc::interrupted) {
